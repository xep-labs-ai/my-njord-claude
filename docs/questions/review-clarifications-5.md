...
**Proposal:**
- `quota_raw` — DecimalField(max_digits=20, decimal_places=4) — handles large raw KB/KIB values
...
**Proposal:** Update `virtual-machine.prp.md` InvoiceDailyCost metadata section to: required: `normalized_usage`, `resolved_prices`, `autofilled`; optional: `cpu_count`, `ram_gb`, `disk_gb` (normalized dimension values), `dimension_costs`, `source_snapshot_date`.
