...
- `autofilled` — BooleanField, default False. True if this daily cost row was generated by force-mode autofill for a day with missing ingestion data.
...
Sum all `InvoiceDailyCost.amount` values grouped by `InvoiceLine` → produces `InvoiceLine.total`

**Step 2: Aggregate InvoiceLine totals into Invoice total**

Sum all `InvoiceLine.total` values → produces `Invoice.total`
